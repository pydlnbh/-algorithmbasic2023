...
public class Code01_SubsequenceMaxModM {
	...
	public static int max4(int[] arr, int m) {
		...
		process4(arr, 0, mid + 1, arr.length - 1, m, sortSet2);
		int ans = 0;
	...
	public static void process4(int[] arr, int sum, int index, int end, int m, TreeSet<Integer> sortSet) {
		if (index == end + 1) {
			sortSet.add(sum % m);
		} else {
			process4(arr, sum, index + 1, end, m, sortSet);
			process4(arr, sum + arr[index], index + 1, end, m, sortSet);
		}
	}
