...
public class Code01_AVLTreeMap {
	...
	public static class AVLTreeMap<K extends Comparable<K>, V> {
		...
		public AVLNode<K, V> rightRotate(AVLNode<K, V> cur) {
			...
			left.r = cur;
			cur.h = Math.max(
				(cur.l != null ? cur.l.h : 0),
			    (cur.r != null ? cur.r.h : 0))
				+ 1;
			left.h = Math.max(
			    (left.l != null ? left.l.h : 0), 
			    (left.r != null ? left.r.h : 0))
				+ 1;
			return left;
		}
		...
		public AVLNode<K, V> leftRotate(AVLNode<K, V> cur) {
			...
			cur.h = Math.max(
				...
				+ 1;
			right.h = Math.max(
			    (right.l != null ? right.l.h : 0), 
			    (right.r != null ? right.r.h : 0))
				+ 1;
			return right;
		}
		...
		private AVLNode<K, V> delete(AVLNode<K, V> cur, K key) {
			...
			if (cur != null) {
				cur.h = Math.max(
					cur.l == null ? 0 : cur.l.h,
					cur.r == null ? 0 : cur.r.h)
				    + 1;
			}
	...
	public static void test() {
		...
		for (int i = 0; i < testTimes; i++) {
			...
			if (treeMap.containsKey(queryK)) {
				...
				Integer key1 = treeMap.floorKey(queryK);
				Integer key2 = avl.floorKey(queryK);
				
				if ((key1 == null && key2 != null) 
					|| (key1 != null && key2 == null)
					|| (key1.intValue() != key2.intValue())) {
					System.out.println("Oops2");
					break;
				}

				key1 = treeMap.ceilingKey(queryK);
				key2 = avl.ceilingKey(queryK);
